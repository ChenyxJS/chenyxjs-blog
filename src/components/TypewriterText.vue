...
<template>
    <div class="typewriter-text" :style="{ color: color }">{{ text }}r</div>
</template>
...
.typewriter-text {
    ...
    &::after {
        ...
        animation: typewriter v-bind(time) steps(v-bind(strLen)) forwards
            infinite;
    }
    &::after {
        width: 5px;
        ...
        animation: typewriter v-bind(time) steps(v-bind(strLen)) forwards
                infinite,
            flicker 0.8s ease-out forwards infinite;
    }
}
